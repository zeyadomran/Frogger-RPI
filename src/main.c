	...
	for(int i = 0; i < CELLSY; i++) {
		for(int j = 0; j < CELLSX; j++) {
			...
			if(state.gameMap[i][j] == CREATORS) {
				...
				loadIMG(creators_banner.height, creators_banner.width, creatorPtr, pixel);
			}
			if(state.gameMap[i][j] == TITLE) {
				short int *titlePtr=(short int *) title_banner.pixel_data;
				loadIMG(title_banner.height, title_banner.width, titlePtr, pixel);
			}
			if(state.gameMap[i][j] == STARTGAME) {
				short int *startPtr=(short int *) startButton_inactive.pixel_data;
				loadIMG(startButton_inactive.height, startButton_inactive.width, startPtr, pixel);
			}
			if(state.gameMap[i][j] == QUITGAME) {
				short int *quitPtr=(short int *) quitButton_inactive.pixel_data;
				loadIMG(quitButton_inactive.height, quitButton_inactive.width, quitPtr, pixel);
			}
		}
	}
...
void loadIMG(unsigned int height, unsigned width, short int* ptr, Pixel *pixel) {
	int i=0;
